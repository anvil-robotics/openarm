...
class BusMock:
    ...
    def send(self, msg: can.Message, _timeout: float | None = None) -> None:
        ...
        assert self.send_index < len(self.expected_sends), (
            f"Unexpected send call. Got: {msg}"
        )

        expected = self.expected_sends[self.send_index]
        assert msg.arbitration_id == expected.arbitration_id, (
            f"Send arbitration_id mismatch. "
            f"Expected: {expected.arbitration_id}, Got: {msg.arbitration_id}"
        )
    ...
    def recv(
        self, arbitration_id: int, _timeout: float | None = None
    ) -> can.Message | None:
    ...
    def verify_all_expectations(self) -> None:
        ...
        """
        assert self.send_index == len(self.expected_sends), (
            f"Not all expected sends called. "
            f"Expected: {len(self.expected_sends)}, Called: {self.send_index}"
        )
        assert self.recv_index == len(self.expected_recvs), (
            f"Not all expected recvs called. "
            f"Expected: {len(self.expected_recvs)}, Called: {self.recv_index}"
        )
