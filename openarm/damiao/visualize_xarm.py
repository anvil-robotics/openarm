...
JOINT_NAMES = ["joint1", "joint2", "joint3", "joint4", "joint5", "joint6", "gripper_fix"]
...
URDF_TO_INTERNAL_NAME_MAP = {
    ...
    "joint5": "joint5",
    "joint6": "joint6",
    "gripper_fix": "gripper_fix",
}

# --- PyBullet Configuration ---
END_EFFECTOR_LINK_NAME = "xarm_gripper_base_link"
...
class PyBulletVisualizer:
    ...
    def __init__(self, urdf_path: str, use_gui: bool = True, log_level: str = "warning"):
        ...
        self.end_effector_link_indices = {'left': 7, 'right': 7}  # End effector links for both arms
    ...
    def setup(self) -> bool:
        ...
        try:
            if should_suppress_output:
                with suppress_stdout_stderr():
                    self.robot_ids['left'] = p.loadURDF(self.urdf_path, [0.2, 0, 0], [0, 0, -0.7071,0.7071], useFixedBase=1)
            else:
                self.robot_ids['left'] = p.loadURDF(self.urdf_path, [0.2, 0, 0], [0, 0, -0.7071,0.7071], useFixedBase=1)
        except p.error as e:
        ...
        try:
            if should_suppress_output:
                with suppress_stdout_stderr():
                    self.robot_ids['right'] = p.loadURDF(self.urdf_path, [-0.2, 0, 0], [0, 0, -0.7071,0.7071], useFixedBase=1)
            else:
                self.robot_ids['right'] = p.loadURDF(self.urdf_path, [-0.2, 0, 0], [0, 0, -0.7071,0.7071], useFixedBase=1)
        except p.error as e:
    ...
    def _map_joints(self) -> bool:
        ...
        for arm_name, robot_id in self.robot_ids.items():
            ...
            mapped_count = 0
            logger.info(f"maps URDF_TO_INTERNAL_NAME_MAP: {URDF_TO_INTERNAL_NAME_MAP}")
